...
def api_system_stats(request):
    """
    API realtime мониторинга системы
    """
    ...
    try:
        ...
        return {
            ...
            "memory_used_mb": round(
                float(memory.used / 1024 / 1024),
                2
            ),

            "memory_total_mb": round(
                float(memory.total / 1024 / 1024),
                2
            ),
...
def save_plot_to_file(plt_figure, filename):
    ...
    try:
        ...
        return f"{settings.MEDIA_URL}charts/{filename}?v={ts}"
    except Exception as e:
...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        if api_key == API_KEY:
            log_info('Successful authentication')
            request.session["authenticated"] = True
            messages.success(request, "Доступ разрешен!")
            return redirect("monitor")
        else:
            log_warning('Failed authentication attempt')
...
def api_get_datasets(request):
    ...
    if request.method == "GET":
        try:
            ...
            log_info(
                f'Generating {num_datasets} datasets | '
                f'Samples={samples_per_dataset}'
            )
            ...
            elif system_stats["memory_percent"] > 50:
