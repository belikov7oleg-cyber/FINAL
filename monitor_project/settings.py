...
# В dev-окружении не перенаправляем HTTP→HTTPS (Django dev server не поддерживает SSL)
...
# Доверенные источники для CSRF — включая варианты с портом для Replit dev-proxy
...
if _dev_domain:
    CSRF_TRUSTED_ORIGINS += [
        f'https://{_dev_domain}',
        f'https://{_dev_domain}:3000',
        f'http://{_dev_domain}',
        f'http://{_dev_domain}:3000',
    ]